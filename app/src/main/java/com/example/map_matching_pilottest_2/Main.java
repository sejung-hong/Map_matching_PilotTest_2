...
/*그림 그려 주는 tool
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        int testNo = 1; // 여기만 바꿔주면 됨 (1-세정, 2-유네, 3-유림)
        FileIO fileIO = new FileIO("dir"); //임의로 변경해놓음
        // 파일에서 읽어와 도로네트워크 생성
        ...
        for (int i = 0; i < roadNetwork.nodeArrayList.size(); i++) {
            ...
            for (int j = 0; j < adjacentLink.size(); j++) {
                AdjacentNode addNode = new AdjacentNode(roadNetwork.getNode(adjacentLink.get(j).second), adjacentLink.get(j).first);
...
//main은 MainActivity에서 구현할 예정이므로 모두 다 주석 처리 했음
